...
router.delete(`/:id`, async (req, res) => {
  ...
  if (!project) {
    res.status(400).json({ message: "Could not find project by that ID" });
  } else {
    try {
      res
        .status(200)
        .json({ message: `Project ${project.name} deleted sucessfully` });
    } catch (err) {
      res
        .status(500)
        .json({ message: "could not delete project at this time" });
    }
  }
});
